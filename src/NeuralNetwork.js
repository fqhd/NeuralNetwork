...
NeuralNetwork.prototype.train = function(input, targets) {
     ...
     let outputDeltas = Matrix.multiply(gradients, hiddenT);

     // Adjusting the output weights and biases
     this.outputWeights.add(outputDeltas);
     this.outputBias.add(gradients);

     let transposedOutputWeights = Matrix.transpose(this.outputWeights);
     let hiddenErrors = Matrix.multiply(transposedOutputWeights, outputErrors);
     ...
     // Adjust the hidden weights and biasses
